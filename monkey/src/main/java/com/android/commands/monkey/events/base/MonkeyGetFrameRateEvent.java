...
public class MonkeyGetFrameRateEvent extends MonkeyEvent {
    ...
    private void writeAverageFrameRate() {
        ...
        try {
            ...
            Logger.warningPrintln("Can't write sdcard log file" + e);
        } finally {
            try {
                if (writer != null)
                    writer.close();
            } catch (IOException e) {
                Logger.warningPrintln("IOException " + e.toString());
            }
        }
    }
    ...
    public int injectEvent(IWindowManager iwm, IActivityManager iam, int verbose) {
        ...
        try {
            ...
            if (status != 0) {
                Logger.println(String.format("Shell command %s status was %s", GET_FRAMERATE_CMD, status));
            }
            ...
            Logger.warningPrintln("Exception from " + GET_FRAMERATE_CMD + ":");
